...
public class UserDao {
	...
	VALUES(SEQ_USER_NO.NEXTVAL, ?, ?, ?, DEFAULT) """;
			...
			// 5. SQL(INSERT) 수행(excuteUpdate()) 후
		...
		try {
			...
			if(rs.next()) { // COUNT(*) 그룹 함수 결과 1행만 조회
				count = rs.getInt(1); // 조회된 컬럼 순서를 이용해 
									  // 컬럼 값 얻어오기 : 1 대신 COUNT(*) 쓰는걸 추천(지금은 조회된게 1행 뿐이라 괜찮)
